...
async function main() {
  ...
  console.log("Press contact:", pressContact);
  ...
  const contact = await prisma.contact.create({
    data: {
      workspaceId: ws.id, campaignId: campaign.id, assignedToId: user.id, kind: "buyer",
      name: pressContact?.name ?? sponsor, title: pressContact?.title,
      email: "", organization: sponsor, status: "NEW",
    },
  });

  const dealContext: BuyerDealContext[] = deals.map(d => ({ dealType: d.dealType, buyer: d.buyer, target: d.target, platform: d.platform, seller: d.seller, industry: d.industryLabel, summary: d.summary }));
  const email = await generateBuyerEmail({ buyerSystemPrompt, sponsor, deals: dealContext, contactName: pressContact?.name.split(" ")[0] });

  const outreach = await prisma.outreach.create({ data: { contactId: contact.id, campaignId: campaign.id, userId: user.id, type: "INITIAL", subject: email.subject, subjectVariants: email.subjectVariants, bodyHtml: email.bodyHtml, bodyPlain: email.bodyPlain, hookUsed: email.hookUsed, tone: email.tone, personalizationScore: personScoreMap[email.personalizationScore], status: OutreachStatus.DRAFT_CREATED, aiMetadata: { buyerOutreach: true, sponsor, pressContact: pressContact ? { ...pressContact } : null } } });

  console.log("E2E OK. sponsor:", sponsor, "| outreachId:", outreach.id, "| contact.name:", contact.name, "| contact.title:", contact.title);
